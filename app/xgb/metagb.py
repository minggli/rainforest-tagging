...
@multithreading
def materialise_data(meta_batch, label_batch):
    ...
    while True:
        ...
        label_flat = np.array([array for nested in label for array in nested])
...
y_pred = np.zero_like(y_test_sample)
for label_index in tqdm(range(17), miniters=1):
